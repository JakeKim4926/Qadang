    ...
    <div class="search-results-container">
      <table>
        ...
        <tbody>          
          ...
          <td v-if="!activeButtons[drink.drinkId]">
            <template v-if="(parseFloat(drink.drinkSugar) + parseFloat(accumulateStore.getAccumulateToday.accumulateSugar)) >= parseFloat(userStore.getUserRDISugar)/2.0">
          ...
          <td v-if="!activeButtons[drink.drinkId]">
            <template v-if="(parseFloat(drink.drinkCaffeine) + parseFloat(accumulateStore.getAccumulateToday.accumulateCaffeine)) >= parseFloat(userStore.getUserRDICaffeine)/2.0">
...
const sortedDrinks = computed(() => {
  ...
  return drinksToSort.sort((a, b) => {
    let result = 0;
    if (a[sortState.sortField] < b[sortState.sortField]) result = -1;
    else if (a[sortState.sortField] > b[sortState.sortField]) result = 1;
...
const saveSortState = () => {
  localStorage.setItem('sortField', sortState.sortField);
  localStorage.setItem('sortOrder', sortState.sortOrder);
};
const loadSortState = () => {
  const savedSortField = localStorage.getItem('sortField') || 'drinkId';
  const savedSortOrder = localStorage.getItem('sortOrder') || 'asc';
  sortState.sortField = savedSortField;
  sortState.sortOrder = savedSortOrder;
};
...
const closeSortMenuOnOutsideClick = (event) => {  
  if (!event.target.closest('.sort-menu') && !event.target.closest('.button_select2')) {
    showSortMenu.value = false;
  }
};
